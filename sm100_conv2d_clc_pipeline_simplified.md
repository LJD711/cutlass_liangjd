...
```
**注意**:PipelineCLCFetchAsync的构造函数会将full_barrier_的arrive_count设置为1，但会将empty_barrier_的设置为consumer_arv_count,这个值与各个CAT中不同角色的warp有关(==一个warp就要乘以32==)(硬件以 SIMT 方式让一个 warp 发射一条指令，32 个 active lane 仍然产生 32 次 arrival 语义)
...
```text
符号：
  B_r = CTA r 的 full_barrier[stage]
  R_r = CTA r 的 clc_response[stage]
  E_0 = CTA0 的 empty_barrier[stage]

┌─────────────────────────────────────────────────────────────────────┐
│ 请求方 active cluster：CTA0 的 Sched warp                          │
└─────────────────────────────────────────────────────────────────────┘
                              │
                              │ 1. producer_acquire(stage)
                              │
                              │    等待 CTA0 的 E_0 ready
                              │    确保上一轮所有 consumer 已读完 response
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────────┐
│ 2. CTA0 Sched warp 为每个 CTA 的 full barrier 登记 16B              │
│                                                                     │
│   lane 0 ── mapa(cta_id=0) ──→ B_0.arrive_expect_tx(16B)            │
│   lane 1 ── mapa(cta_id=1) ──→ B_1.arrive_expect_tx(16B)            │
│   lane 2 ── mapa(cta_id=2) ──→ B_2.arrive_expect_tx(16B)            │
│   lane 3 ── mapa(cta_id=3) ──→ B_3.arrive_expect_tx(16B)            │
│   其余 lane：pred=false，不操作                                     │
│                                                                     │
│   每个 B_r：arrival = 0，pending transaction = 16B                  │
│                                                                     │
│   PTX：                                                             │
...
└─────────────────────────────────────────────────────────────────────┘
                              │
                              │ 3. elect_one_sync()
                              │    CTA0 Sched warp 只选一个 lane
                              ▼
┌─────────────────────────────────────────────────────────────────────┐
│ 4. elected lane 发出唯一一条 CLC query                              │
│                                                                     │
│   clusterlaunchcontrol.try_cancel.async.shared::cta                 │
│     .mbarrier::complete_tx::bytes                                   │
│     .multicast::cluster::all.b128                                   │
│                                                                     │
│   尝试取消并接手一个尚未启动的完整 cluster                          │
└─────────────────────────────────────────────────────────────────────┘
                              │
                              │ CLC 硬件异步执行
                              ▼
┌─────────────────────────────────────────────────────────────────────┐
│ 5. 将同一个 16B response multicast 到请求方 cluster 的所有 CTA      │
│                                                                     │
│   写 R_0 ── complete_tx(16B) ──→ B_0 ready                          │
│   写 R_1 ── complete_tx(16B) ──→ B_1 ready                          │
│   写 R_2 ── complete_tx(16B) ──→ B_2 ready                          │
│   写 R_3 ── complete_tx(16B) ──→ B_3 ready                          │
│                                                                     │
│   每个 CTA 的 full barrier 独立完成                                 │
└─────────────────────────────────────────────────────────────────────┘
                              │
           ┌──────────────────┼──────────────────┐
           │                  │                  │
           ▼                  ▼                  ▼
┌──────────────────┐ ┌──────────────────┐ ┌──────────────────────────┐
│ CTA0 consumers   │ │ CTA1 consumers   │ │ CTA2/CTA3 consumers      │
│                  │ │                  │ │                          │
│ consumer_wait(B0)│ │ consumer_wait(B1)│ │ consumer_wait(B2/B3)     │
│        │         │ │        │         │ │           │              │
│        ▼         │ │        ▼         │ │           ▼              │
│ read R0          │ │ read R1          │ │ read R2/R3               │
│ ld.shared.b128   │ │ ld.shared.b128   │ │ ld.shared.b128           │
│        │         │ │        │         │ │           │              │
│        ▼         │ │        ▼         │ │           ▼              │
│ 解析 valid       │ │ 解析 valid       │ │ 解析 valid               │
│ /first_ctaid     │ │ /first_ctaid     │ │ /first_ctaid             │
│        │         │ │        │         │ │           │              │
│        ▼         │ │        ▼         │ │           ▼              │
│ 每个参与线程     │ │ 每个参与线程     │ │ 每个参与线程             │
│ consumer_release │ │ consumer_release │ │ consumer_release         │
└────────┬─────────┘ └────────┬─────────┘ └────────────┬─────────────┘
         │                    │                        │
         │                    │                        │
         │  mapa.shared::cluster(..., cta_id=0)       │
         │  mbarrier.arrive.shared::cluster.b64       │
         │                    │                        │
         └────────────────────┼────────────────────────┘
                              ▼
┌─────────────────────────────────────────────────────────────────────┐
│ 6. 所有 consumer arrival 汇聚到 CTA0 的 E_0                        │
│                                                                     │
│   CTA0 Sched threads ───────────────┐                               │
│   CTA0 Mainloop/MMA/Epilogue ───────┤                               │
│   CTA1 Mainloop/MMA/Epilogue ───────┤──→ CTA0.empty_barrier[stage]  │
│   CTA2 Mainloop/MMA/Epilogue ───────┤                               │
│   CTA3 Mainloop/MMA/Epilogue ───────┘                               │
│                                                                     │
│   每个参与线程贡献一次 arrival：                                    │
│                                                                     │
│   pending_count                                                     │
│     consumer_arv_count → ... → 2 → 1 → 0                            │
└─────────────────────────────────────────────────────────────────────┘
                              │
                              │ 最后一个 arrival
                              ▼
┌─────────────────────────────────────────────────────────────────────┐
│ 7. E_0 完成当前 phase，empty barrier ready                          │
│                                                                     │
│   含义：整个 cluster 的所有 consumer 都已把 response 读入寄存器     │
│   结果：clc_response[stage] 可以安全覆盖                            │
└─────────────────────────────────────────────────────────────────────┘
                              │
                              │ 下一轮 producer_acquire(stage)
                              ▼
                    返回步骤 1，发下一次 query
...
producer_get_barrier()
    取得 full_barrier[stage] 的 SMEM 地址
        ...
        ▼
producer_acquire()
    等 empty_barrier[stage] (这一步要等待所有的warp到达arrive)
    给 cluster 内每个 CTA 的 full barrier 登记 16B
        │
        ▼
elect_one_sync()
    只让一个 lane 发 query(因为这里只有一个lane发射，所以full_barrier.arrive_count要设为1)
        │
        ▼
issue_clc_query()
    尝试取消未启动 cluster
...
```text
consumer_wait(full[stage])(这里每个CTA都会去等自己的mbarrier的信号，因为之前CTA0的部分lane为每个CAT的mbarrier设置了tx=16Byte，到了16B才会放行)
        │
        ▼
...
CLC query
   ├─ 写入 CTA0.clc_response[stage] → B0 complete_tx(16B)
   ├─ 写入 CTA1.clc_response[stage] → B1 complete_tx(16B)
   ├─ 写入 CTA2.clc_response[stage] → B2 complete_tx(16B)
   └─ 写入 CTA3.clc_response[stage] → B3 complete_tx(16B)
每个 CTA 的写入完成后，对应 barrier 的 transaction count 变为 0
...
- 直接调用阻塞式 `producer_acquire()`；
- 直接调用阻塞式 `consumer_wait()`；
- 不调用 `producer_commit()`，因为 CLC query 自带 `mbarrier::complete_tx::bytes`。

如果显式调用 `producer_commit()`，其底层是：

```ptx
mbarrier.complete_tx.shared::cluster.relaxed.cluster.b64
  [full_barrier], transaction_bytes;
```

它属于 pipeline 通用接口，不属于本 CLC 调用链的实际执行路径。
...
CTA0 Sched warp
  advance_to_next_work()
    ├─ wait empty[stage]
    ├─ full[stage].arrive_and_expect_tx(16B)，覆盖所有 CTA
    └─ clusterlaunchcontrol.try_cancel.async...b128
                              │
                              ▼
                  CLC response multicast 到所有 CTA
                  full[stage] transaction complete
                              │
                              ▼
所有参与角色
  fetch_next_work()
    ...
    ├─ swizzle_and_rasterize
    └─ arrive empty[stage]@CTA0
                              │
                              ▼
                  所有 consumer arrival 到齐
                  empty[stage] ready
                              │
                              ▼
                     下一次 CLC query

query 失败：next_work.valid=false
  → 各 warp 完成当前 work
  → Sched warp producer_tail()
  → kernel 结束
```

一句话概括：

> full barrier 保证“CLC response 已经到达，可以读取”；empty barrier 保证“所有角色已经读完，可以覆盖”；`clusterlaunchcontrol.try_cancel` 负责原子地认领尚未启动 cluster 的工作，并把 16B 结果广播给当前 cluster。
